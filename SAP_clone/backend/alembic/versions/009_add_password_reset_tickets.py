...
def upgrade():
    ...
    result = conn.execute(sa.text(
        "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'password_reset_tickets')"
    ))
    ...
    op.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_password_reset_tickets_id ON password_reset_tickets (id)"))
    op.execute(sa.text("CREATE UNIQUE INDEX IF NOT EXISTS ix_password_reset_tickets_sap_ticket_id ON password_reset_tickets (sap_ticket_id)"))
    op.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_password_reset_tickets_servicenow_ticket_id ON password_reset_tickets (servicenow_ticket_id)"))
    op.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_password_reset_tickets_username ON password_reset_tickets (username)"))
    op.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_password_reset_tickets_correlation_id ON password_reset_tickets (correlation_id)"))
